...
## BÖLÜM 0: KRİTİK BAŞLANGIÇ NOTLARI

### 5 Kırmızı Çizgi (asla ihlal etmeyin)

1. **Gün 1 akşamı varsayım testleri tamamlanmadan kod yazmaya geçmeyin.** Tavily'nin Trendyol'dan kaç yorum çektiği, forum filtreleme çalışıyor mu, Gemini Vision video kesit alıyor mu — bu üçü bilinmeden mimari kararlar verilemez.

2. **Gün 5 sonunda end-to-end akış çalışmıyorsa, polish değil özellik kesin** Yeni özellik eklemeyi bırak, mevcut akışı sağlamlaştır.

3. **Gün 7 sonunda feature freeze.** Sadece bug fix, test, dokümantasyon. Yeni satır kod yazılmaz, sadece silinir veya düzenlenir.

4. **Demo ürünleri sabitleninceye kadar dashboard üzerinde "demo modu" toggle'ı olsun.** Pre-cache'lenmiş veriyi anında yükleyen mod. Demo videosu çekerken canlı API çağrısına bel bağlamayın.

5. **Sahte yorum dilini her yerde yumuşatın.** "Sahte tespit edildi" → "Yüksek şüphe sinyali". UI'da, API response'unda, README'de, demo'da. Hukuki risk + jüri ML soruları için zorunlu.

### Bugün (11 Mayıs) bitmeden mutlaka cevap bulunması gereken 3 soru

| # | Soru | Test | Beklenen Cevap | Cevap "Hayır"sa |
|---|------|------|----------------|-----------------|
| 1 | Tavily Trendyol yorumlarından kaç tane snippet çekebiliyor? | `tavily.search("airpods pro yorum site:trendyol.com")` | 10+ unique yorum snippet | SerpAPI'ye geç veya Selenium düşün |
| 2 | Tavily forum filtrelemesi çalışıyor mu? | `include_domains=["donanimhaber.com","technopat.net","sikayetvar.com","eksisozluk.com"]` | 5+ farklı thread linki | Manuel curated forum URL listesi yap |
| 3 | Gemini 2.5 Pro 30 saniyelik video kesitini analiz edebiliyor mu? | YouTube'dan 30sn kes, base64 yükle, "ürün ekranındaki sayıyı oku" sor | Doğru cevap + < 15sn süre | Sadece transkript moduna düş |
...
## BÖLÜM 2: KARAR VERME NOKTALARI (HER GÜN SABAH SYNC'TE KONTROL)

### Checkpoint 1 — Gün 2 sabahı (12 Mayıs)
**Soru:** Dün gece varsayım testlerinin sonucu ne?
- ✅ 3/3 yeşil → Plana devam
- ⚠️ 2/3 yeşil → Kırmızı varsayım için Plan B'yi BUGÜN devreye al
- ❌ 1/3 veya 0/3 yeşil → DUR. Mimariyi gözden geçir, scope yarıya indir

### Checkpoint 2 — Gün 4 sabahı (14 Mayıs)
**Soru:** End-to-end ham pipeline çalışıyor mu? (1 ürünle giriş → karar)
- ✅ Evet → P1 özelliklerine başla (multimodal, görsel doğrulama deep dive)
- ❌ Hayır → P0'ı bitir, P1/P2 ertele veya tamamen çıkar

### Checkpoint 3 — Gün 6 sabahı (16 Mayıs)
**Soru:** Tüm P0 + en az 3 P1 hazır mı?
- ✅ Evet → Gün 7-8: Polish + test + dokümantasyon
- ❌ Hayır → Hangi P1'leri kesiyorsun? **Tutarsız sonuç vermek**, eksik özellikten beterdir

### Checkpoint 4 — Gün 7 sabahı (17 Mayıs)
**FEATURE FREEZE.** Bu sabahtan sonra:
- Yeni özellik = HAYIR
- Bug fix = EVET
- UI polish = EVET (sınırlı)
- Test, demo prep, dokümantasyon = ÖNCELİK

---

## BÖLÜM 3: 9 GÜNLÜK DETAYLI SPRINT
